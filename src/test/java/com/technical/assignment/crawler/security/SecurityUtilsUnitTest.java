package com.technical.assignment.crawler.security;
...
class SecurityUtilsUnitTest {
    ...
    void testGetCurrentUserLoginForJWT() {
        ...
        Jwt jwtToken = new Jwt(ID_TOKEN, Instant.now(), Instant.now().plusSeconds(60), headers, claims);
        securityContext.setAuthentication(new JwtAuthenticationToken(jwtToken));
        SecurityContextHolder.setContext(securityContext);
        Optional<String> login = SecurityUtils.getCurrentUserLogin();
        assertThat(login).contains("admin");
    }
